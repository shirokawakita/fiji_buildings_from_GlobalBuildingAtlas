...
### 6.5 DPA/DDD → 単位体積あたり欠陥数密度の求め方

PHITSのDPA/DDD出力から、材料ごとの単位体積あたり欠陥数密度（Frenkel pair density, cm⁻³）を算出する方法を説明します。
...
**化合物（GaAs、CIGS、SiO₂等）**では、式量中の原子数を掛けて原子密度を得ます。

**PHITS v3.35の新機能**: PHITS v3.35では、`[T-DPA]`タリーに`mother`パラメータが導入されており、元素別DPAを直接出力できます。これは化合物半導体での元素別検証に有用です。
...
`scripts/defect_density.py`を使用して、PHITS出力から欠陥数密度を自動計算できます。
...
**パラメータの説明**:
- `--mode`: `dpa`（DPAから換算、推奨）または`ddd`（DDDから近似換算）
- `--input`: PHITS出力ファイル（per-source値）
- `--fluence`: 総フルエンス [cm⁻²]
- `--cells`: 対象セルID（複数指定可能）
- DPAモード用: `--rho`（密度 [g/cm³]）、`--M`（モル質量 [g/mol]）、`--atoms_per_formula`（化学式あたりの原子数）
- DDDモード用: `--rho`（密度 [g/cm³]）、`--Ed`（はじき出ししきい値 [eV]）、`--eta`（欠陥生成効率、デフォルト0.8）
- `--csv_out`: 出力CSVファイル名

**注意**: `scripts/defect_density.py`の簡易パーサは**2列（cell_id, value_per_source）**を想定しています。PHITS出力形式が異なる場合は、スクリプトをカスタマイズしてください。
...
**ARC-DPAはNRT-DPAより小さくなる傾向**があります。これは、高エネルギー衝突で生成された欠陥が非熱的に再結合する効果を考慮しているためです。条件によっては、ARC-DPAの方が実験値により近い結果を与える場合があります。
